...
It is also possible to rotate an individual service by passing the `--service` flag, for example: `rke2 certificate rotate --service api-server`. See link:./security/certificates.md#rotating-client-and-server-certificates-manually[Certificate Management] for more details.
...
RKE2 performs these checks because nameserver addresses of these types would not function properly within pods that https://kubernetes.io/docs/concepts/services-networking/dns-pod-service/#pod-s-dns-policy[inherit name resolution configuration] from their node.
