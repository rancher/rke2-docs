...
. The `rke2-selinux` policy module is installed and loaded on the host.
. RKE2 itself is started with SELinux support enabled (`selinux: true` in the config file, or `RKE2_SELINUX=true` in the environment).
...
If RKE2 was installed from the tarball _before_ `rke2-selinux`, the files on disk carry whatever contexts they inherited at extraction time, and the policy will not have been applied to them. Installing the policy afterwards does not retroactively relabel them.
...
Installing `rke2-selinux` and setting `selinux: true` are separate steps and neither implies the other. RKE2 logs a warning at startup when they disagree -- see <<_troubleshooting,Troubleshooting>>.
...
| https://github.com/rancher/rke2-selinux/blob/v0.23.stable.1/policy/centos9/rke2.te#L22-L25[`rke2_service_db_t`]
| The `etcd` static pod
| Read-write access to the datastore under `/var/lib/rancher/rke2/server/db`

| https://github.com/rancher/rke2-selinux/blob/v0.23.stable.1/policy/centos9/rke2.te#L13-L17[`rke2_service_t`]
| The remaining control-plane static pods
| Read-only access to control-plane state

| https://github.com/rancher/rke2-selinux/blob/v0.23.stable.1/policy/centos9/rke2.te#L34-L37[`rke2_tls_t`]
| `/var/lib/rancher/rke2/server/tls`
| Certificates and keys read by the control-plane pods
|===
...
* Istio fails by default on `Enforcing` nodes. See link:../known_issues.md#istio-in-selinux-enforcing-system-fails-by-default[Known Issues].
