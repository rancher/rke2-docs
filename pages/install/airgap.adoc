...
. Assuming amd64, use `docker image load -i rke2-images.linux-amd64.tar.zst` to import images from the tar file into docker.
...
. You can save the Hauler store content directly to the images directory. For RKE2, ensure the directory `/var/lib/rancher/rke2/agent/images/` exists on the node, then save the tarbarll to that path:
...
. Follow the RKE2 <<_2_install_rke2,install instructions>>. Extract files needed for the install from the Hauler store. Use `hauler store info` to identify the exact artifact names in your store.
...
See link:../security/selinux.md#air-gapped-and-offline-installs[Air-gapped and offline installs] for how to download the policy and its dependencies, and xref:../security/selinux.adoc[SELinux] for the rest of the SELinux requirements.
...
. On a networked machine, download the install script, rke2 binary tarball, images archive, and sha256sum into a directory:
+
[,bash]
----
...
. Copy `/root/rke2-artifacts` (or your chosen directory) to each air-gapped node, for example with `scp`, USB media, or your approved offline transfer path. Keep the same relative file names.
. On each air-gapped node, run install.sh against that directory:
+
[,bash]
----
...
. Follow the steps of the link:../upgrades/manual.md#manually-upgrade-rke2-using-the-binary[manual upgrade method]
...
RKE2 supports xref:../upgrades/automated.adoc[automated upgrades]. To enable this in air-gapped environments, you must ensure the required images are available in your private registry.
...
Once you have added the necessary rancher/rke2-upgrade, rancher/system-upgrade-controller, and rancher/kubectl images to your private registry, follow the xref:../upgrades/automated.adoc[automated upgrades] guide.
