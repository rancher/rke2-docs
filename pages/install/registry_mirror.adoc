...
Images imported via link:airgap.md?airgap-load-images=Manually+Deploy+Images#prepare-the-images-directory-and-airgap-image-tarball[air-gap image tar files] or link:../add-ons/import-images.md#pre-import-images[pre-imported] are pinned in containerd to
...
Images loaded directly into containerd via link:./airgap.md?airgap-load-images=Manually+Deploy+Images[air-gap tarballs], link:../add-ons/import-images.md#pre-import-images[pre-imported] or loaded directly into containerd's image store using the `ctr` command line tool,
...
or by loading image archives created by `docker save` via the `ctr -n k8s.io image import` command or the link:../add-ons/import-images.md#pre-import-images[pre-import feature].
