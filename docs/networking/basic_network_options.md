...
Cilium does not support encapsulation for IPv6 traffic; communication between pods on different nodes relies on the network to properly route packets. Cilium can be configured to add routes between nodes with the following configuration:
